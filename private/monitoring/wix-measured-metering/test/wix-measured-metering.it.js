...
describe('WixMeasuredMetering IT', () => {
  
  describe('raw', () => {
    
    it('should report hist and meter upon success', done => {
      const {reporter, measuredClient} = measuredClientWithReporter();
      const metering = new WixMeasuredMetering(measuredClient);
      const {reportDuration} = metering.raw('key', 'value');
      
      setTimeout(() => {
        reportDuration(27);
        const reportedHist = reporter.hists('key=value').toJSON();
        const reportedMeter = reporter.meters('key=value').toJSON();
        
        expect(reportedMeter.count).to.equal(1);
        expect(reportedHist.count).to.equal(1);
        expect(reportedHist.median).to.be.equal(27);
        done();
      }, 20);
    });
    
    it('should report error rate upon failure', () => {
      const {reporter, measuredClient} = measuredClientWithReporter();
      const metering = new WixMeasuredMetering(measuredClient);
      const returnedError = new MyBusinessError('woop');
      const {reportError} = metering.raw('key', 'value');

      reportError(returnedError);

      const reporterMeter = reporter.meters('key=value.error=MyBusinessError.code=-150').toJSON();
      return expect(reporterMeter.count).to.equal(1);
    });
  });
