...
eval(global.Opal.Opal.ERB.$compile(Buffer.from('__TEMPLATE__', 'base64').toString('utf8'), 'my-template'))

const ctx = global.Opal.Object.$new()
const json = Buffer.from('__FIELDS__', 'base64').toString('utf8')
const jsFields = JSON.parse(json)
const rubyFields = global.Opal.JSON.$parse(json)
...
output = global.Opal.Template['$[]']('my-template').$render(ctx)
