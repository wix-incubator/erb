...
module.exports = opts => (context, apps) => {
  ...
  if (runMode.isProduction() && process.env['WIX-BOOT-EXPRESS-SEEN-BY']) {
    log.debug(`production mode detected, env variable 'WIX-BOOT-EXPRESS-SEEN-BY' set, skipping loading from config.`);
    effectiveOptions.seenBy = process.env['WIX-BOOT-EXPRESS-SEEN-BY'];
  } else if (runMode.isProduction()) {
    log.debug(`production mode detected, loading seen-by from config: ${context.env.confDir}/${configName}.json.erb`);
