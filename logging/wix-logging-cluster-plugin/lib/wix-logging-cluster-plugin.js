...
function Logger() {
  ...
  this.onMaster = (cluster, next) => {
    server.onMessage(evt => {
      let stream = (_.includes(['debug', 'info'], evt.level)) ? process.stdout : process.stderr;
...
const validators = [
  ...
  event => (_.isEmpty(event.level) || !_.includes(levels, event.level)) ? `level is mandatory and must be one of [${levels.join(', ')}]` : undefined,
