...
describe('wix web context aspect', () => {
  ...
  describe('import', () => {
    ...
    it('should import x-seen-by headers', () => {
      ...
      aspect.import({
        headers: {
          'x-seen-by': ['another-seen-by', 'yet-another-seen-by, yet-yet-another-seen-by']
        }
      });

      expect(aspect.seenBy).to.deep.equal(['some-seen-by', 'another-seen-by', 'yet-another-seen-by', 'yet-yet-another-seen-by']);
    });
    ...
    it('should be safe with undefined import object', () => {
      const aspect = build({
        url: 'http://fanta.wixpress.com/woop',
        remotePort: 1233,
        remoteAddress: '127.0.2.2'
