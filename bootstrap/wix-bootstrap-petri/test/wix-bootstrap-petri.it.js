...
describe('wix-bootstrap-petri', function () {
  this.timeout(60000);

  const app = appWithSetup();

  it('should return a fallback if experiment is not active', () =>
    conductExperiment('anExperiment')
      .then(res => expect(res).to.equal('false'))
  );

  it('should conduct feature toggle experiment', () =>
    givenFeatureToggle('scope', 'anExperiment')
      .then(() => conductExperiment('anExperiment'))
      .then(res => expect(res).to.equal('true'))
  );

  it('should conduct AB test experiment', () =>
  ...
  function conductExperiment(key, authenticated) {
    ...
    return http.okGet(app.getUrl(`/conduct/experiment/${key}?fallback=false`), opts)
      .then(res => res.text())
  }

  function conductAllInScope(scope, authenticated) {
    const opts = authenticated ? reqOptions.builder().withBi().withSession().options() : {};
    return http.okGet(app.getUrl(`/conduct/scope/${scope}`), opts)
      .then(res => res.json())
  }
